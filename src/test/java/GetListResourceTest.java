import io.restassured.http.ContentType;
import org.testng.annotations.Test;
import rest.dto.listResource.Datum;
import rest.dto.listResource.Root;
import rest.dto.listResource.Support;

import java.util.List;

import static io.restassured.RestAssured.given;
import static org.assertj.core.api.Assertions.assertThat;

public class GetListResourceTest {

    @Test
    public void getListResourceTest() {
        Root expectedRoot = Root.builder()
                                .page(1)
                                .per_page(6)
                                .total(12)
                                .total_pages(2)
                                .data(List.of(Datum.builder()
                                                   .id(1)
                                                   .name("cerulean")
                                                   .year(2000)
                                                   .color("#98B2D1")
                                                   .pantone_value("15-4020").build(),
                                        Datum.builder()
                                             .id(2)
                                             .name("fuchsia rose")
                                             .year(2001)
                                             .color("#C74375")
                                             .pantone_value("17-2031").build(),
                                        Datum.builder()
                                             .id(3)
                                             .name("true red")
                                             .year(2002)
                                             .color("#BF1932")
                                             .pantone_value("19-1664").build(),
                                        Datum.builder()
                                             .id(4)
                                             .name("aqua sky")
                                             .year(2003)
                                             .color("#7BC4C4")
                                             .pantone_value("14-4811").build(),
                                        Datum.builder()
                                             .id(5)
                                             .name("tigerlily")
                                             .year(2004)
                                             .color("#E2583E")
                                             .pantone_value("17-1456").build(),
                                        Datum.builder()
                                             .id(6)
                                             .name("blue turquoise")
                                             .year(2005)
                                             .color("#53B0AE")
                                             .pantone_value("15-5217").build()))
                .support(Support.builder()
                        .url("https://reqres.in/#support-heading")
                        .text("To keep ReqRes free, contributions towards server costs are appreciated!").build()).build();

        Root actualRoot = given().
                                  contentType(ContentType.JSON).
                                  accept(ContentType.JSON).
                                  log().ifValidationFails().
                           when().
                                  get("https://reqres.in/api/unknown").
                           then().
                                  statusCode(200).
                                  log().ifValidationFails().
                           extract().
                                     body().as(Root.class);

        assertThat(actualRoot).isEqualTo(expectedRoot);
    }
}